    ...
    def generate_new_list(self):
        if self.shuffle:
            ...
            if self.replace:
                ...
                # label_cnter = Counter(self.dataset.data_source.labels.numpy())
                # weights = [1.0 / label_cnter[label] for label in list(self.dataset.data_source.labels.numpy())]
                # indices = torch.multinomial(torch.from_numpy(np.array(weights)), len(self.dataset), replacement=True).tolist()
                # print(Counter(self.dataset.data_source.labels.numpy()[indices]))
                # print(indices)
        else:
...
class DistributedGivenIterationSampler(Sampler):
    ...
    def set_uniform_indices(self, labels, num_classes):
        ...
        N = len(labels)
        print('------------------------')
        print('sadasdasddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddd')
        size_per_label = int(N / num_classes) + 1
        indices = []
        images_lists = [[] for i in range(num_classes)]
        for i, l in enumerate(labels):
