...
class Custom_base(metaclass=ABCMeta):
    ...
    def __init__(self,
                 root, split, return_label=True, data_type="classification", seed=42):
        self.root = root
        self.split = split
...
class ProcessedDataset(Custom_base):
    ...
    def __init__(self,
                 root, split, return_label=True, data_type="classification", seed=42):
        super().__init__(root, split, return_label, data_type, seed)
    ...
    def load_data(self):
        for name in scandir(self.root, self.EXTENSIONS, False):
            seed = name.split(".")[0].split("_")[-1]
            if(seed== str(self.seed)):
